...
| `tushare_provider.go` | `TushareProvider`：Tushare 真实数据源（骨架，待实现） |
...
- `TushareProvider` — 构造函数 `NewTushareProvider(client *tushare.Client) *TushareProvider`，使用 Tushare API（待完善）
...
| `domain/market` → `pkg/tushare` | Tushare 客户端（仅 TushareProvider） |
